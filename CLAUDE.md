...
- **String resources**: Add new strings ONLY to default `strings.xml` (in `values/`). NEVER add strings to German locale files like `values-de` or `values-de-rAT` (`strings-de` / `strings-de-at`).
