...
class Preferencemanager(
    ...
    /**
     * Drops the session credentials (both tokens, own id) and the legacy push key, nothing else. Logout calls this first,
    ...
    // Events tab: last selected list/day/upcoming/month view. Stored by enum name (not ordinal) so
    ...
    fun getEventsViewModeFlow(): Flow<EventsViewMode> = prefs.data.map { prefs ->
        ...
        EventsViewMode.entries.firstOrNull { it.name == saved } ?: EventsViewMode.UPCOMING
    }
